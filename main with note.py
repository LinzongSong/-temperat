...
class Ui_Form(object):
 ...
 # Топологическая сортировка БЗ
    def topological(self):
        Q = []
        P = []
        ans = "" # Сортировка результатов
    ...
    def go(self, flag=True):
        # Включать генеративных правил в базу знаний
        # if P then Q
        # Чтение файл
        self.Q = []
        self.P = []
        ...
        flag = True
        temp = ""
        for x in self.P:  # Для каждого правила
            if ListInSet(x, self.DB):  # Если все это есть в базе знаний
                self.DB.add(self.Q[self.P.index(x)])
                temp = self.Q[self.P.index(x)]
                flag = False  # По крайней мере, один вывод
                # print("%s --> %s" %(x, self.Q[self.P.index(x)]))
                self.str += "%s --> %s\n" % (x, self.Q[self.P.index(x)])

        if flag:    # Никаких выводов не может быть достигнута
            print("can not reason a single conclusion")
            for x in self.P:  # Для каждого правила
                if ListOneInSet(x, self.DB): #Удовлетворяют ли факты оторым посылкам
                    flag1 = False  # Отрицание предпосылки при запросе по умолчанию
                    for i in x:    # Для всех элементов помещения
                        if i not in self.DB: # Для той части, которая не увлетворена
                            btn = s.quest("is it " + i)
                            if btn == QtWidgets.QMessageBox.Ok:
                                self.textEdit.setText(self.textEdit.toPlainText() + "\n" + i)  # ОК увеличивается до textEdit
                                self.DB.add(i)  # OK добавляется в базу правил
                                flag1 = True
                                # self.go(self)
                    if flag1:   #Если посылка положительна, то повторно умозаключать
                        self.go()
                        return
...
# Запросить дополнительные факты
    ...
    def handle_click(self):
        if not self.isVisible():
            ...
            # Содержимое текстового поля вывода
